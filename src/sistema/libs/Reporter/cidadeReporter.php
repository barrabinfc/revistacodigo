...
class cidadeReporter extends Reporter
{
	...
	// 'CustomFieldExample' is an example that is not part of the `city` table
	...
	*/
	static function GetCustomQuery($criteria)
	{
		$sql = "select
			...
			,`city`.`state` as State
			,`city`.`institution` as Institution
			,`city`.`citypublic` as Citypublic
			,`city`.`city` as City
	...
	*/
	...
	{
		$sql = "select count(1) as counter from `city`";
