...
var page = {
	...
	renderModelView: function(showDeleteButton)	{
		...
		// populate the dropdown options for state
		// TODO: load only the selected value, then fetch all options when the drop-down is clicked
		var stateValues = new model.estadoCollection();
		stateValues.fetch({
			success: function(c){
				var dd = $('#state');
				...
				c.forEach(function(item,index) {
					dd.append(app.getOptionHtml(
						item.get('idstate'),
						item.get('state'), // TODO: change fieldname if the dropdown doesn't show the desired column
						page.cidade.get('state') == item.get('idstate')
					));
				});
			...
			success: function(c){
				...
				c.forEach(function(item,index) {
					dd.append(app.getOptionHtml(
						...
						page.cidade.get('institution') == item.get('idinstitution')
					));
				});
	...
	updateModel: function() {
		...
		page.cidade.save({
			...
			'citypublic': $('input#citypublic').val(),
			'city': $('input#city').val(),
			'citycode': $('input#citycode').val()
		}, {
			wait: true,
			success: function(){
