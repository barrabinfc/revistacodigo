/**
 * View logic for descricao_items
 */
...
var page = {
	...
	descricao_items: new model.descricao_itemCollection(),
	collectionView: null,
	descricao_item: null,
	modelView: null,
	isInitialized: false,
	...
	init: function() {
		...
		this.collectionView = new view.CollectionView({
			...
			collection: page.descricao_items
		});
		...
		$('#filter').change(function(obj) {
			...
			page.fetchdescricao_items(page.fetchParams);
		});
		...
		this.collectionView.on('rendered',function(){
			...
			$('table.collection tbody tr').click(function(e) {
				e.preventDefault();
				var m = page.descricao_items.get(this.id);
		...
		this.fetchdescricao_items({ page: 1 });
		...
		if (model.longPollDuration > 0)	{
			setInterval(function () {
				...
				if (!page.dialogIsOpen)	{
					page.fetchdescricao_items(page.fetchParams,true);
				}
	 ...
	 */
	fetchdescricao_items: function(params, hideLoader) {
		...
		page.descricao_items.fetch({

			data: params,

			success: function() {

				if (page.descricao_items.collectionHasChanged) {
			...
			success: function(c){
				...
				c.forEach(function(item,index) {
					dd.append(app.getOptionHtml(
						...
						page.descricao_item.get('tem') == item.get('iditem')
					));
				});
	...
	updateModel: function() {
		...
		page.descricao_item.save({
			...
			success: function(){
				...
				if (isNew) { page.descricao_items.add(page.descricao_item) }
